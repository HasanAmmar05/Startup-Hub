...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Add additional check for user ID
    if (!session?.user?.id) {
      ...
      <section className="profile_container">
        <div className="profile_card">
          <div className="profile_title">
            <h3 className="text-24-black uppercase text-center line-clamp-1">{userData.name}</h3>
          </div>
          ...
          <p className="text-30-extrabold mt-7 text-center">@{userData.username}</p>
          <p className="mt-1 text-center text-14-normal">{userData.bio}</p>
        </div>

        <div className="flex-1 flex flex-col gap-5 lg:-mt-5">
          <p className="text-30-bold">{session.user.id === params.id ? "Your" : "All"} Startups</p>
